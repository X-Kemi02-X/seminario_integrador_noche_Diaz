...
function mostrarProducto(p: Producto): void {
  ...
  console.log(`[${p.idE}] ${p.nombre} $${p.precio}${desc} (${stock})`);
}
...
function ProductoEmpleado(e: Empleado): void {
  console.log(`[${e.idE}] ${e.nombre} - ${e.apellido} - ${e.cargo} - ${e.ubicacion}`);
}
...
ProductoEmpleado(Empleado1);
ProductoEmpleado(Empleado2);