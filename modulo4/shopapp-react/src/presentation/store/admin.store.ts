...
export const useAdminStore = create<AdminState & AdminActions>((set, get) => ({
  ...
  async fetchUsers(page?: number, search?: string) {
    const nextPage = page ?? get().usersPage
    const nextSearch = search ?? get().usersSearch
    set({ isLoadingUsers: true, usersError: null, usersPage: nextPage, usersSearch: nextSearch })
    try {
      const data = await userUseCase.getUsers(nextPage, nextSearch || undefined)
      set({ users: data.results, usersTotal: data.count })
    } catch {
  ...
  async toggleUserActive(id) {
    try {
      const result = await userUseCase.toggleUserActive(id)
      set({ users: get().users.map((u) => (u.id === id ? { ...u, is_active: result.is_active } : u)) })
